...
constexpr GrdTypeKind grd_make_type_kind(const char (&str)[N]) {
	GrdTypeKind kind = 0;
	for (int i = 0; i < (N - 1); i++) {
...
const char* grd_type_kind_as_c_str(GrdTypeKind* kind) {
...
GrdPrimitiveValue grd_make_primitive_value(auto thing) {
	GrdPrimitiveValue v = {};
...
struct GrdEnumType: GrdType {
	...
	bool                          is_flags  = false;
	GrdReflectArray<GrdEnumValue> values;
};
...
struct GrdStructType: GrdType {
	...
	GrdReflectArray<GrdStructMember> unflattened_members;
	GrdReflectArray<GrdStructMember> members;
	const char*                      subkind = "";
};
...
struct GrdMapType: GrdType {
	...
	GrdGenerator<GrdItem*> (*iterate)      (void* map) = NULL;
};
...
struct GrdFunctionType: GrdType {
	...
	GrdReflectArray<GrdType*> arg_types;
};

template <typename T>
GrdPrimitiveType* grd_reflect_primitive_type(const char* name, GrdPrimitiveKind kind);

#define GRD_REFLECTION_REFLECT_PRIMITIVE(TYPE, KIND) GrdPrimitiveType* grd_grd_reflect_type(TYPE* x, GrdPrimitiveType* type) {\
	type->name = #TYPE;\
...
GrdPrimitiveType* grd_reflect_type(bool* x, GrdPrimitiveType* type);

template <typename T, s64 N>
GrdFixedArrayType* grd_reflect_type(T (*arr)[N], GrdFixedArrayType* type);

template <typename T> requires(!std::is_function_v<T>)
GrdPointerType* grd_reflect_type(T** thing, GrdPointerType* type);

template <typename T> requires(std::is_function_v<T>)
GrdFunctionType* grd_reflect_type(T* thing, GrdFunctionType* type);
...
template <typename T, u64 N>
void grd_reflect_hook(GrdReflectHookIndexTag<N> tag, GrdEmptyStruct x = {}) {
	if constexpr (N == 0) {
...
template <typename T>
concept GrdMemberReflect = requires(T a) { 
	{ T::grd_reflect_type(&a, NULL) };
};

template <typename T>
concept GrdGlobalReflect = requires(T a) { 
	{ grd_reflect_type(&a, NULL) };
};
...
inline static GrdReflection GRD_REFLECT;

template <typename T>
struct GrdTypeMapper {
	inline static GrdType* type = NULL;
};

template <typename T>
constexpr u32 grd_reflect_get_type_size() {
	return sizeof(T);
}

template <>
constexpr u32 grd_reflect_get_type_size<void>() {
	return 0;
}

template <typename T, typename TypeT>
TypeT* grd_reflect_add_type(TypeT* type) {
...
template <typename T, typename TypeT>
TypeT* grd_reflect_add_type_named(const char* name) {
	GrdScopedLock(GRD_REFLECT.lock);
	static TypeT type;
	grd_reflect_add_type<T, TypeT>(&type);
	type.name = name;
	return &type;
}

template <GrdMemberReflect T>
GrdType* grd_reflect_type_internal() {
	using TypeT = std::remove_cvref_t<decltype(*T::grd_reflect_type((T*) NULL, NULL))>;
	auto type = grd_reflect_add_type_named<T, TypeT>("");
	return T::grd_reflect_type((T*) NULL, type);
}

template <GrdGlobalReflect T>
	requires (!GrdMemberReflect<T>)
GrdType* grd_reflect_type_internal() {
	using TypeT = std::remove_cvref_t<decltype(*grd_reflect_type((T*) NULL, NULL))>;
	auto type = grd_reflect_add_type_named<T, TypeT>("");
	return grd_reflect_type((T*) NULL, type);
}

// Every type pointer auto casts to void*, that's why
//  we can't have global grd_reflect_type(void*, ) to reflect type 'void'.
template <typename T> requires (std::is_same_v<T, void>)
GrdType* grd_reflect_type_internal() {
	auto* type = grd_reflect_add_type_named<void, GrdPrimitiveType>("");
	type->name = "void";
...
template <typename T>
GrdType* grd_reflect_type_internal() {
	auto type = grd_reflect_add_type_named<T, GrdUnregisteredType>("");
...
template <typename T>
GrdType* grd_reflect_type_of() {
	GrdScopedLock(GRD_REFLECT.lock);
	using XT = grd_remove_all_const<T>::type;
	using Mapper = GrdTypeMapper<XT>;
	if (!Mapper::type) {
		auto type = grd_reflect_type_internal<XT>();
...
const char* grd_resolve_type_name_alias(const char* name) {
...
GrdFunctionType* grd_reflect_type(R (**function)(Args...), GrdFunctionType* type) {
	// @TODO: set name.
...
GrdType* grd_reflect_get_pointer_inner_type_with_indirection_level(GrdPointerType* pointer_type, s32* result_indirection_level) {
...
bool grd_reflect_does_type_inherit(GrdType* derived, GrdType* base, s32* out_offset) {
...
bool grd_reflect_can_cast(GrdType* derived, GrdType* base, s32* out_offset) {
	if (derived == base) {
		*out_offset = 0;
		return true;
	}
...
void grd_reflect_flatten_struct_members(GrdType* type, s32 offset, GrdStructMember member) {
...
void grd_reflect_struct_type_add_member(GrdType* type, GrdStructMember member) {
	if (type->kind != GrdStructType::KIND) {
		return;
	}
	auto t = (GrdStructType*) type;
...
void grd_reflect_enum_type_add_value(GrdType* type, GrdEnumValue v) {
...
void grd_reflect_add_tag(GrdType* type, auto value) {
...
#define GRD_REFLECT_NAME(_T, _name)\
template <typename __T = _T> requires(std::is_convertible_v<__T, _T>)\
inline static GrdReflectMacroPickType<__T>* grd_reflect_type(__T* x, GrdReflectMacroPickType<__T>* type) {\
	type->name = _name;\
	grd_reflect_fill_type(x, type);\
	grd_reflect_maybe_flatten_struct_type(type);\
	return type;\
}\
inline static void grd_reflect_fill_type(_T* x, GrdReflectMacroPickType<_T>* type)
...
GrdType* grd_get_real_type(GrdType* type, void* ptr) {
	if (ptr != NULL) {
		if (type->kind == GrdStructType::KIND) {
			auto casted = (GrdStructType*) type;
...
struct GrdReflectArrayType: GrdSpanType {
	s64 (*get_capacity)(void* arr);
};

template <typename T>
GrdReflectArrayType* grd_reflect_type(GrdReflectArray<T>* x, GrdReflectArrayType* type) {
	type->inner = grd_reflect_type_of<T>();
	type->subkind = "reflect_array";
	type->name = grd_heap_sprintf("GrdReflectArray<%s>", type->inner->name);
	type->get_count = [](void* arr) {
		return ((GrdReflectArray<T>*) arr)->count;
	};
	type->get_capacity = [](void* arr) {
		return ((GrdReflectArray<T>*) arr)->capacity;
	};
	type->get_item = [](void* arr, s64 index) -> void* {
		return ((GrdReflectArray<T>*) arr)->operator[](index);
	};
	return type;
}
