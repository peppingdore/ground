...
constexpr GrdSpan<T> grd_make_string(const T* data, s64 length) {
	return { (T*) data, length };
}

template <GrdStringChar T>
GrdSpan<T> grd_make_string(const T* c_str) {
	auto ptr = c_str;
	while (true) {
		if (ptr[0] == '\0') {
			break;
...
constexpr GrdString operator""_b(const char* c_str, size_t length) { 
	return { (char*) c_str, (s64) length };
}

constexpr GrdUnicodeString operator""_b(const char32_t* c_str, size_t length) {
	return { (char32_t*) c_str, (s64) length };
}

template <GrdStringChar T>
void grd_append(GrdArray<T>* arr, GrdSpan<T> str, s64 index = -1, GrdCodeLoc loc = grd_caller_loc()) { 
...
void grd_append(GrdArray<char32_t>* arr, GrdString str, s64 index = -1, GrdCodeLoc loc = grd_caller_loc()) {
	auto ptr = grd_reserve(arr, grd_len(str), index, loc);
	for (auto i: grd_range(grd_len(str))) {
		ptr[i] = str[i];
	}
...
template <s64 N>
void grd_append(GrdArray<char32_t>* arr, const char32_t (&str)[N], s64 index = -1, GrdCodeLoc loc = grd_caller_loc()) {
	grd_add(arr, GrdSpan<char32_t>{ str, N - 1 }, index, loc);
}

template <s64 N>
void grd_append(GrdArray<char32_t>* arr, const char (&str)[N], s64 index = -1, GrdCodeLoc loc = grd_caller_loc()) {
...
template <s64 N>
void grd_append(GrdArray<char>* arr, const char (&str)[N], s64 index = -1, GrdCodeLoc loc = grd_caller_loc()) {
	grd_add(arr, GrdSpan<char>{ str, N - 1 }, index, loc);
}

void grd_append(GrdArray<char32_t>* arr, const char32_t* str, s64 index = -1, GrdCodeLoc loc = grd_caller_loc()) {
	auto s = grd_make_string(str);
	grd_add(arr, s, index, loc);
}

void grd_append(GrdArray<char32_t>* arr, const char* str, s64 index = -1, GrdCodeLoc loc = grd_caller_loc()) {
	auto s = grd_make_string(str);
	grd_append(arr, s, index, loc);
}

void grd_append(GrdArray<char>* arr, const char* str, s64 index = -1, GrdCodeLoc loc = grd_caller_loc()) {
	auto s = grd_make_string(str);
	grd_add(arr, s, index, loc);
}
...
template <GrdStringChar T>
GrdArray<T> grd_copy_string(GrdSpan<T> str, GrdCodeLoc loc = grd_caller_loc()) {
	return grd_copy_string(c_allocator, str, loc);
}

GrdAllocatedUnicodeString grd_copy_unicode_string(GrdAllocator allocator, GrdString str) {
	GrdArray<char32_t> result = { .allocator = allocator };
	grd_append(&result, str);
	return result;
}

GrdAllocatedUnicodeString grd_copy_unicode_string(GrdString str) {
	return grd_copy_unicode_string(c_allocator, str);
}

bool grd_is_whitespace(char32_t c) {
...
bool grd_is_line_break(char32_t c) {
...
template <GrdStringChar T>
bool grd_is_blank(GrdSpan<T> str) {
	for (T c: str) {
...
bool grd_starts_with(GrdSpan<T> str, GrdSpan<U> start) {
...
bool grd_starts_with(GrdSpan<T> str, const U (&start)[N]) {
	return grd_starts_with(str, GrdSpan<U>{ (U*) start, N - 1 });
}

template <GrdStringChar T, GrdStringChar U>
bool grd_ends_with(GrdSpan<T> str, GrdSpan<U> end) {
...
char32_t grd_ascii_to_lower(char32_t c) {
	return (c >= 'A' && c <= 'Z') ? c + ('a' - 'A') : c;
}

template <GrdStringChar T, GrdStringChar U>
bool grd_compare_ignore_case_ascii(GrdSpan<T> a, GrdSpan<U> b) {
...
template <GrdStringChar T>
GrdSpan<T> grd_trim_start(GrdSpan<T> str) {
	for (auto i: grd_range(grd_len(str))) {
...
template <GrdStringChar T>
GrdSpan<T> grd_trim_end(GrdSpan<T> str) {
...
template <GrdStringChar T>
GrdSpan<T> grd_trim(GrdSpan<T> str) {
...
template <GrdStringChar T>
GrdTuple<GrdSpan<T>, bool> grd_take_until(GrdSpan<T> str, auto should_stop) {
	for (auto i: grd_range(grd_len(str))) {
...
template <GrdStringChar T>
GrdGenerator<GrdSpan<T>> grd_split(GrdSpan<T> str, auto predicate) {
	s64 cursor = 0;
	for (auto i: grd_range(grd_len(str))) {
		if (predicate(str[i])) {
			auto x = str[cursor, i];
...
template <GrdStringChar T>
GrdTuple<GrdSpan<T>, GrdSpan<T>> grd_split2(GrdSpan<T> str, auto predicate) {
	for (auto i: grd_range(grd_len(str))) {
		if (predicate(str[i])) {
...
template <GrdStringChar T>
s64 grd_get_line_break_len(GrdSpan<T> str, s64 idx) {
	if (idx < grd_len(str)) {
...
template <GrdStringChar T>
GrdGenerator<GrdSpan<T>> grd_iterate_lines(GrdSpan<T> str, bool include_line_breaks = true) {
	s64 cursor = 0;
	s64 i = 0;
	while (i < grd_len(str)) {
...
template <GrdStringChar T, GrdStringChar U>
GrdSpan<T> grd_remove_prefix(GrdSpan<T> str, GrdSpan<U> prefix) {
...
template <GrdStringChar T, GrdStringChar U>
GrdSpan<T> grd_remove_suffix(GrdSpan<T> str, GrdSpan<U> suffix) {
...
s64 grd_utf8_length(GrdUnicodeString str) {
	s64 length = 0;
	for (auto i: grd_range(grd_len(str))) {
...
void grd_encode_utf8(GrdUnicodeString str, char* buf) {
	char* ptr = buf;
	for (auto i: grd_range(grd_len(str))) {
		auto c = str[i];
...
GrdAllocatedString grd_encode_utf8(GrdAllocator allocator, GrdUnicodeString str, GrdCodeLoc loc = grd_caller_loc()) {
...
GrdAllocatedString grd_encode_utf8(GrdUnicodeString str, GrdCodeLoc loc = grd_caller_loc()) {
	return grd_encode_utf8(c_allocator, str, loc);
}

s32 grd_utf16_char_size(char32_t c) {
	return c >= 0x10000 ? 2 : 1;
}

GrdArray<char16_t> grd_encode_utf16(GrdAllocator allocator, GrdUnicodeString str, GrdCodeLoc loc = grd_caller_loc()) {
...
GrdArray<char16_t> grd_encode_utf16(GrdUnicodeString str, GrdCodeLoc loc = grd_caller_loc()) {
	return grd_encode_utf16(c_allocator, str, loc);
}

GrdAllocatedUnicodeString grd_decode_utf8(GrdAllocator allocator, GrdString utf8, GrdCodeLoc loc = grd_caller_loc()) {
...
GrdAllocatedUnicodeString grd_decode_utf8(GrdAllocator allocator, const char* c_str, GrdCodeLoc loc = grd_caller_loc()) {
	return grd_decode_utf8(allocator, grd_make_string(c_str), loc);
}

GrdAllocatedUnicodeString grd_decode_utf8(const char* c_str, GrdCodeLoc loc = grd_caller_loc()) {
	return grd_decode_utf8(c_allocator, c_str, loc);
}

GrdAllocatedUnicodeString grd_decode_utf16(GrdAllocator allocator, const char16_t* str, s64 length = -1, GrdOptional<GrdByteOrder> bo = {}, GrdCodeLoc loc = grd_caller_loc()) {
...
GrdAllocatedUnicodeString grd_decode_utf16(const char16_t* str, s64 length = -1, GrdOptional<GrdByteOrder> bo = {}, GrdCodeLoc loc = grd_caller_loc()) {
