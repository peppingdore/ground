...
inline void print_integer_number_to_char_buffer_reversed(auto num, int base, bool uppercase, char* buf, auto* cursor) {
...
inline void print_integer_number_to_char_buffer(auto number, int base, bool uppercase, char* buffer, auto* cursor) {
	...
	print_integer_number_to_char_buffer_reversed(number, base, uppercase, buffer, cursor);
	reverse(buffer + start_position, *cursor - start_position);
}
...
inline SmallString to_string(T num, IntegerStringParams p = {}) {
	...
	if (!p.skip_base_prefix) {
		switch (p.base) {
			case 2:  append(&res, p.uppercase ? "0B" : "0b"); break;
			case 8:  append(&res, "0"); break;
			case 16: append(&res, p.uppercase ? "0X" : "0x"); break;
		}
	}
...
inline SmallString to_string(T num, int max_decimal_digits = 99999999) {
	...
	if (exponent != 0) {
		print_integer_number_to_char_buffer_reversed(abs(exponent), 10, false, res.buf, &res.length);
	...
	print_integer_number_to_char_buffer_reversed(integral_part, 10, false, res.buf, &res.length);

	if (negative) {
		append(&res, '-');
	}

	reverse(res.buf, res.length);
	return res;
}
...
#pragma pop_macro("min")
	...
	T number = bitcast<T>(accumulator);
...
inline bool parse_float(BaseString<Char> str, f64* result, Float_Parsing_Params params = {}) {
	...
	s32 significand_part_end = (e_position != -1) ? e_position : str.length;
	s32 significand_length = significand_part_end - significand_part_start;
	auto significand_part = make_string(str.data + significand_part_start, significand_length);
	...
	// Parse significand.
	{
		...
		for (auto i: range(significand_part.length)) {
			...
			if (c == '_') {
				if (params.allow_underscores) {
					if ((i > 0) && (i < significand_part.length - 1)) {
						continue;
					}
				}
	...
	if (e_position != -1) {
		auto   exponent_string = slice(str, e_position + 1);
		s32    exp = 1;
		...
		if (exponent_string.length > 0) {
			if (exponent_string[0] == '-') {
				...
				exponent_string = slice(exponent_string, 1);
		...
		for (auto i: reverse(range(exponent_string.length))) {

			auto c = exponent_string[i];
			if (c == '_') {
				if (params.allow_underscores) {
					if ((i > 0) && (i < exponent_string.length - 1)) {
						continue;
					}
				}
				return false;
...
inline bool parse_float(auto str, f32* result, Float_Parsing_Params params = {}) {
	f64 num;
	bool success = parse_float(str, &num, params);
	if (success) {
		*result = num;
	}
	return success;
