...
inline bool parse_integer(Span<Char> str, T* result, ParseIntegerParams params = {}) {
...
inline bool parse_float(Span<Char> str, f64* result, Float_Parsing_Params params = {}) {
	if (len(str) <= 0) {
		return false;
	}
	...
	s32 significand_part_end = (e_position != -1) ? e_position : len(str);
	s32 significand_length = significand_part_end - significand_part_start;
	Span<Char> significand_part = { str.data + significand_part_start, significand_length }; 
		...
		for (auto i: range(len(significand_part))) {
			...
			if (c == '_') {
				if (params.allow_underscores) {
					if ((i > 0) && (i < len(significand_part) - 1)) { 
						continue;
					}
				}
	...
	if (e_position != -1) {
		...
		for (auto i: reverse(range(len(exponent_string)))) {
			...
			if (c == '_') {
				if (params.allow_underscores) {
					if ((i > 0) && (i < len(exponent_string) - 1)) {
						continue;
					}
				}
				return false;
