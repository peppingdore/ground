...
GRD_DEDUP GrdMul128Result grd_mul128(u64 op1, u64 op2) {
...
GRD_DEDUP u64 grd_wy_rand_64(u64* state) {
...
GRD_DEDUP u64 GRD_DEFAULT_RANDOM_SEED = 0x49c6e7b299f58cca;

struct RandomState {
	u64 wy_state = GRD_DEFAULT_RANDOM_SEED;
};

GRD_DEDUP thread_local RandomState grd_thread_local_random_state;

GRD_DEDUP RandomState grd_make_random_state(u64 seed = GRD_DEFAULT_RANDOM_SEED) {
	return { .wy_state = seed };
}

GRD_DEDUP u64 grd_rand_u64(RandomState* state = &grd_thread_local_random_state) {
	return grd_wy_rand_64(&state->wy_state);
}

GRD_DEDUP s64 grd_rand_s64(RandomState* state = &grd_thread_local_random_state) {
	return grd_bitcast<s64>(grd_rand_u64(state));
}

GRD_DEDUP u64 grd_rand_range_u64_state(RandomState* state, u64 range_length) {
	if (range_length == 0) {
		return 0;
	}
...
GRD_DEDUP u64 grd_rand_range_u64(u64 range_length) {
	return grd_rand_range_u64_state(&grd_thread_local_random_state, range_length);
}

GRD_DEDUP u64 grd_rand_range_u64_state(RandomState* state, u64 start, u64 end_exclusive) {
...
GRD_DEDUP u64 grd_rand_range_u64(u64 start, u64 end_exclusive) {
	return grd_rand_range_u64_state(&grd_thread_local_random_state, start, end_exclusive);
}

GRD_DEDUP u64 grd_rand_map_signed_to_unsigned(s64 num) {
	if (num >= 0) {
...
GRD_DEDUP s64 grd_rand_map_unsigned_to_signed(u64 num) {
	if (num >= s64_max) {
...
GRD_DEDUP s64 grd_rand_range_s64_state(RandomState* state, s64 start, s64 end_exclusive) {
...
GRD_DEDUP s64 grd_rand_range_s64(s64 start, s64 end_exclusive) {
